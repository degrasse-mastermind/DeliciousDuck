...
2. `.github/workflows/codex-task.yml` validates the contract in a short preparation job and exports only safe routing metadata before the time-limited Codex job begins. Codex revalidates the same contract before combining it with the selected business charter and delivery-mode prompt.
...
5. A stable `conversation_key` does not authorize transmission. Only a contract with a separately approved callback and matching dispatch flag may send one result. The callback receives a pre-callback reconciliation containing the terminal Codex and publishing outcomes, a separate terminal task status, callback `attempting`, and reconciliation `pending-callback-reconciliation`. It uses its contract-bound idempotency key, requests a trigger run ID, safely classifies HTTP failures, and polls the accepted run to `completed` or `failed` without logging credentials or unrestricted response bodies. After that single callback, the bridge writes the callback-inclusive final reconciliation to the linked GitHub issue. Implementation changes complete only after the publishing job succeeds and returns a valid GitHub pull-request URL; a publishing or callback failure cannot leave the durable ledger labeled successful.
...
The validated machine contract is the complete source of operative instructions. Every specialist question, output limit, acceptance criterion, and prohibition must appear in `objective`, `scope`, `out_of_scope`, or `acceptance_criteria`. The linked issue supplies ledger provenance and receives the result; it must not be the only source of instructions that Codex needs to execute the task.
...
Before transmission, the bridge renders the known Codex and publishing results for the COO. It derives a terminal task status from those results, labels callback status `attempting`, and separately labels reconciliation status `pending-callback-reconciliation`. A Codex or required-publishing failure therefore remains `failed` while callback reconciliation is pending. A callback cannot truthfully include its own terminal outcome before it runs, and the workflow never sends a second callback to add that outcome later.

The callback step writes a safe structured outcome for final reconciliation. The GitHub issue result records `not-approved` when no callback external write was authorized, `not-attempted` only when an approved callback step was actually skipped, `attempted-no-result` when the step failed or was cancelled without structured outputs, `completed` after a terminal successful run, `blocked` for non-runnable, invalid-output, or configuration failures, `timed-out` when polling does not reach a terminal state, and `failed` for a terminal failed run. A structured callback result takes precedence over the step fallback. The overall result is posted after callback handling; an approved callback that does not complete also fails the report job after the reconciled issue comment is written.

Validated issue and callback routing come from the preparation job rather than the time-limited Codex job. If Codex is cancelled, times out, or fails without outputs, the report job retains the approved issue number, conversation key, callback decision, and callback idempotency key. It can therefore record the failure and, only when the task contract separately authorized it, attempt the single idempotent failure callback before failing the workflow overall.

Publishing is reconciled independently. It is `not-required` for read-only modes and implementations with no changes, `completed` only when required publishing succeeds with a valid GitHub pull-request URL, and `failed` otherwise. An existing failure remains failed even when a later callback outcome is merely blocked.
