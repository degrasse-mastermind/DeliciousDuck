...
Before transmission, the bridge renders the known Codex and publishing results for the COO, labels callback status `attempting`, and labels overall status `pending-callback-reconciliation`. A callback cannot truthfully include its own terminal outcome before it runs, and the workflow never sends a second callback to add that outcome later.
