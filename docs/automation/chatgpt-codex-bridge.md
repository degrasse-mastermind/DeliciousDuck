...
5. A stable `conversation_key` does not authorize transmission. Only a contract with a separately approved callback and matching dispatch flag may return the final result. That callback uses its contract-bound idempotency key, requests a trigger run ID, safely classifies HTTP failures, and polls the accepted run to `completed` or `failed` without logging credentials or unrestricted response bodies. The bridge reconciles Codex execution, required implementation publishing, and the callback outcome before commenting on the linked issue. Implementation changes complete only after the publishing job succeeds and returns a valid GitHub pull-request URL; a publishing or callback failure cannot leave the durable ledger labeled successful.
