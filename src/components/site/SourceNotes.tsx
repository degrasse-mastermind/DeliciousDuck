 ...
 */
export function SourceNotes({ ids, extra }: { ids: string[]; extra?: string[] }) {
  const sources = sourceList(ids);
  if (sources.length === 0 && !extra?.length) return null;

  return (
    <section aria-labelledby="sources" className="mt-16 rounded-sm bg-cream p-6 lg:p-7">
      <div className="flex items-center gap-2.5">
        <BookOpen aria-hidden="true" className="size-4 text-primary" />
        <h2 id="sources" className="eyebrow text-primary">
          References
        </h2>
      </div>
